...
use crate::system_io::{fs_create_dir, fs_read, fs_remove_dir, fs_write, get_relative_fw_path};
use log::{info, trace};
...
impl UniversalOverlayHandler {
    ...
    fn vfs_check_applied(&self, path_rel: &str) -> Result<(), FpgadError> {
        let path_file_contents = self.get_vfs_path()?;
        if path_file_contents.contains(path_rel) {
            info!("overlay path contents is valid: '{path_file_contents}'");
        } else {
            return Err(FpgadError::OverlayStatus(format!(
                "When trying to apply overlay '{path_rel}', the resulting vfs path contained '{path_file_contents}'"
            )));
        }
...
impl OverlayHandler for UniversalOverlayHandler {
    ...
    fn apply_overlay(&self, source_path: &Path) -> Result<(), FpgadError> {
        let source_path_rel = get_relative_fw_path(source_path)?;
