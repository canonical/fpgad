#!/usr/bin/env python3
"""Bitstream loading tests for xlnx platform."""
...
class TestBitstreamXlnx(FPGATestBase):
    ...
    def test_load_bitstream_local(self):
        ...
        # dfx-mgr output format: "Loaded with slot_handle X"
        self.assertIn("Loaded with slot_handle", proc.stdout)
    ...
    def test_load_bitstream_path_not_exist(self):
        ...
        # dfx-mgr returns error via softener
    ...
    def test_load_bitstream_bad_flags(self):
        ...
        # Note: dfx-mgr may succeed even with bad flags, just check it doesn't crash
        # The bad flags are for the fpga_manager, not dfx-mgr
        if proc.returncode != 0:
            self.assert_in_proc_err("FpgadError", proc)
