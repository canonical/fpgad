...
async fn call_xlnx_sys_read(sub_cmd: &str, path: &str) -> Result<String, zbus::Error> {
    let connection = Connection::system().await?;
    let proxy = status_proxy::StatusProxy::new(&connection).await?;
    proxy.xlnx_sys(sub_cmd, path).await
}
...
async fn call_xlnx_sys_write(
    ...
    proxy.xlnx_sys(sub_cmd, path, value).await
}
...
/// * `Err(zbus::Error)` - DBus communication error or FpgadError.
///   See [Error Handling](../index.html#error-handling) for details.
pub async fn xlnx_sys_handler(sub_command: &XlnxSysSubcommand) -> Result<String, zbus::Error> {
    match sub_command {
        XlnxSysSubcommand::Read { sub_cmd, path } => call_xlnx_sys_read(sub_cmd, path).await,
        XlnxSysSubcommand::Write {
            sub_cmd,
            path,
            value,
        } => call_xlnx_sys_write(sub_cmd, path, value).await,
    }
}
