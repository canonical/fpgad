...
async fn expected_good_overlay_process(_setup: ()) {
    ...
    let platform_handle: &str = "xlnx-sys";
    ...
    control_proxy
        .xlnx_sys("write_property", "/sys/class/fpga_manager/fpga0/key", "")
        .await
...
async fn argument_errors(
    ...
    let good_platform_handle: &str = "xlnx-sys";
    ...
    // reset flags
    control_proxy
        .xlnx_sys("write_flags", good_device_handle, "0")
        .await
        .expect("failed to set fpga flags");
    expect_that!(
        status_proxy
            .xlnx_sys("read_flags", good_device_handle)
            .await
            .expect("failed to get fpga flags"),
        eq("0"),
        "flags not set properly"
...
#[gtest]
#[rstest]
#[case::bad_dtbo_path("fpga0", "xlnx-sys", "/lib/firmware/bad-dtbo-path")]
async fn io_errors(
    #[case] overlay_handle: &str,
    #[case] platform_handle: &str,
    #[case] overlay_file: &str,
