...
//!
//! This module provides the XilinxSys platform, which is a generic implementation
...
//! - [`XilinxSysFPGA`] - Manages FPGA device operations
//! - [`XilinxSysOverlayHandler`] - Manages overlay operations
//!
//! # Registration
//!
...
/// string "xlnx-sys", making it available as a fallback for devices without specific
/// platform support.
///
/// # Fields
///
...
impl XilinxSysPlatform {
    /// Create a new XilinxSys platform instance.
    ///
    ...
    ///
    /// # Returns: `Self`
    /// * New XilinxSysPlatform instance ready for use
    ///
    /// # Examples
    ///
    /// ```rust,no_run
...
impl Platform for XilinxSysPlatform {
    ...
    ///
    /// Returns a reference to the [`XilinxSysFPGA`] instance for the specified device.
    ...
    ///
    /// Returns a reference to the [`XilinxSysOverlayHandler`] instance for the specified
    ...
    fn overlay_handler(&self, overlay_handle: &str) -> Result<&dyn OverlayHandler, FpgadError> {
        // TODO: replace the return type of XilinxSysOverlayHandler to Result and use
...
///
/// Used as the `sub_cmd` argument in the `xilinx_sys` method of the status DBus interface and
/// in the `xilinx_sys read` CLI subcommand. The string representation (see [`ReadSubCommand::as_str`])
...
///
/// ```rust,no_run
/// use daemon::platforms::xilinx_sys::ReadSubCommand;
///
...
///
/// Used as the `sub_cmd` argument in the `xilinx_sys` method of the control DBus interface and
/// in the `xilinx_sys write` CLI subcommand. The string representation (see [`WriteSubCommand::as_str`])
/// is the value that must be passed over DBus or in the CLI.
///
/// # Variants
///
...
///
/// ```rust,no_run
/// use daemon::platforms::xilinx_sys::WriteSubCommand;
///
